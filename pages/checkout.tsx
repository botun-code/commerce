...
export default function Checkout() {
  ...
  const checkForm = useCallback(
    (entity) => {
      // TODO rewise god bless for this piece of shit
      if (
        (entity.name &&
          entity.name !== '' &&
          entity.name.length > 2 &&
          entity.phone &&
          ((!entity.phone.includes('_') &&
          entity.phone.length === 20) ||
        entity.phone.length === 20 && entity.phone[entity.phone.length - 1] === '_'))
      ) {
        setCanOrder(true)
      } else {
        setCanOrder(false)
  ...
  const handleChange = useCallback(
    ...
    [setFormData, formData, checkForm]
  )
